...
async function getBook() {
    // 10dan katta 20dan kichkina
    // const book = await Book.find({ price: { $gt: 10, $lt: 20 } })
    // faqat 10 15 20 narxli kitoblarni chiqarib ber demoqdamiz
    // const book = await Book.find({ price: { $in: [10, 15, 20] } })
    //.or bu yoki degani shu apratordan foydansak bo'ladi bu huddi || shunaqa degani
    // .and bu && bunga o'xshaydi shunday
    // biz documentni uzunligini bilmoqchi bo'lsak .selectni o'rniga .countDocuments() deb yozsak bo'ladi
    const book = await Book.find()
        .or([{ author: 'IslomUmar' }, { isPublished: true }])
        .limit(2)
        .sort({ name: 1 })
        .countDocuments()
